...
import random
'''
Dit volledig project is gebaseerd op de cursus van Michael Nielsen gegeven op de website neuralnetworksanddeeplearning.com
'''
class Netwerk(object):
    def __init__(self, groottes):
        ...
        [array([[-0.5024744 , -0.26511926],
         ...
         nummers = [1,2,3]
         letters = ['a','b','c']
         zip(nummers,letters) = [(1,'a'),(2,'b'),(3,'c')]
        """
    ...
    def feedforward(self, a):
        ...
        for b, w in zip(self.biases, self.gewichten):
    ...
    def update_mini_batch(self, mini_batch, eta):
        ...
        De mini batch is net als de "totale batch" een lijst tuples "(x, y)", en "eta"
        is de "learning rate"."""
        nabla_b = [np.zeros(b.shape) for b in self.biases] #shape geeft de dimensie van het array b
        nabla_w = [np.zeros(w.shape) for w in self.weights]
        for x, y in mini_batch:
            delta_nabla_b, delta_nabla_w = self.backprop(x, y) #dees functie komt nog lol
            nabla_b = [nb+dnb for nb, dnb in zip(nabla_b, delta_nabla_b)] #de accent versie (nabla b')
